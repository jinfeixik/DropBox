...
<body>
	...
	<div id="right-content" class="right-content"
		...
		<div class="content">
			<div id="page_content">
				<span style="margin-left:24px;color: #999;font-size: 14px;">链接分享(分享失败超过1年以上的链接记录将被自动清理)</span>
				<br />
				<div class="">
					...
					<div class="zuijinTitle cl"
						...
						<a href="javascript:void(0);"
							...
							class="button button-glow button-border button-rounded button-primary"
							style="margin-left: 20px;display:none" id="deleteAll">清空我的分享</a>
						<p class="y time"
							style="margin:10px 115px 0 0;color: #505458;font-size: 14px;">分享时间</p>
						<p class="y size"
							style="margin:10px 32px 0 0;color: #505458;font-size: 14px;">下载次数</p>
						<p class="y size"
							style="margin:10px 32px 0 0;color: #505458;font-size: 14px;">浏览次数</p>
					</div>
					<div class="content cl">
						<c:forEach items="${fileShares }" var="fileShare">
							<div class="template cl" data-click="no">
								...
								<p class="z name" style="margin-top: 15px;">${fileShare.fileInfo.fileName
									}</p>
								<p class="y time" style="margin-top: 15px;">${fileShare.shareTime
									}</p>
								<p class="y size" style="margin: 15px 62px 0 0;font-size:14px;">${fileShare.shareDownloadNumber
									}次</p>
								<p class="y size" style="margin: 15px 65px 0 0;font-size:14px;">${fileShare.shareViewNumber
									}次</p>
								<div id="operation" style="display: none;">
									<a href="javascript:void(0);" id="delete"> <img
										src="images/cancel.png" class="y iconImg"
										style="width: 20px;height: 20px;margin: 15px 30px 0 0;"
										alt="取消分享"> <input type="hidden"
										value="${fileShare.fileId }" /> </a> <a
										href="javascript:void(0);" id="reduction"> <c:if
											test="${fileShare.shareStatus == 0}">
											<img src="images/key_close.png" class="y iconImg"
												style="width: 20px;height: 20px;margin: 15px 30px 0 0;"
												alt="公开">
										</c:if> <c:if test="${fileShare.shareStatus == 1}">
											<img src="images/key_open.png" class="y iconImg"
												style="width: 20px;height: 20px;margin: 15px 30px 0 0;"
												alt="加密">
										</c:if> <input type="hidden" value="${fileShare.fileId }" /> </a>
								</div>
							</div>
						</c:forEach>
					</div>
													...
													$(this)
															...
															.click(
																	function() {
																		...
																		$
																				.ajax({
																					url : "updateShareStatus.form",
																					type : "post",
																					data : {
																						fileId : fileId
																					}
																				});
																		window.location.href = "getAllShare.form";
																	});
												});
								$(this).mouseleave(
										function() {

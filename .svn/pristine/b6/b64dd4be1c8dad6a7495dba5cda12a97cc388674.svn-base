...
public final class FileTypeJudge {  
    ...
    private static String getFileContent(InputStream is) throws IOException {  
        ...
        byte[] b = new byte[28];  
        System.out.println((is == null) + "=========is");
        try {  
        	System.out.println("报错处1" +is.toString());
            is.read(b, 0, 28);  
            System.out.println("报错处2");
        } catch (IOException e) {  
            e.printStackTrace();  
     ...
     * 
     * @param value 表示文件类型
     * @return 1 表示图片,2 表示文档,3 表示视频,4 表示种子,5 表示音乐,0 表示其它
     * @return
     */
    ...
    public static void main(String args[]) throws Exception {  
    	System.out.println("文件后缀名" +(FileTypeJudge.getType(new FileInputStream(new File("D:/aaa/我的简历120190822.pdf")))));
    	 System.out.println("文件类型" +FileTypeJudge.isFileType(FileTypeJudge.getType(new FileInputStream(new File("D:/aaa/我的简历120190822.pdf")))));

							...
							<li role="presentation" style="margin-left: 20px;"><a
								role="menuitem" tabindex="-1" href="#">
									...
									<div class="col-md-8" style="margin-top: 5px;">
										...
										<font size="2" color="#000" style="margin-top: -40px;">
										<c:if test="${memberInfo.memberStatus==0}">
									             普通用户
										</c:if>
		...
		<div class="left-menu">
			...
			<div class="contain_four">
				<div class="contain_four_main">
					<!-- <span>容量：</span>  --><span class="cont">${user.totalSpace}</span>已用：<span class="use">${user.usedSpace}</span>
				</div>
				<div class="max_add">
								...
								<c:forEach items="${fileInfos}" var="fileInfo">
									<div class="fileclick" id="${fileInfo.fileId}">
										<div class="template cl" data-click="no"
											...
											<p class="y size" style="margin-top: 15px;">${fileInfo.fileSize}M</p>

											<a style="margin: 15px 25px 0 0;" class="y"
												href="moveToRecycle.form?fileId=${fileInfo.fileId}"><i
												class="fa fa-trash fa-lg"></i>
											</a> <a style="margin: 15px 25px 0 0;" class="y"  onclick="javascript:share(this.id);" id="${fileInfo.fileId}"><i
												...
												href="${fileInfo.fileTransformPDF}"
												id="a"><span class="fa fa-photo fa-lg"></span>
											</a>
...
<script type="text/javascript">
   ...
   //alert(e);
    $.ajax({
             ...
             area: ['500px','300px'],
             btn: ['确定', '关闭'],
             skin: 'lasyui-layer-demo', //样式类名
             closeBtn: 0, //不显示关闭按钮
             anim: 2,
             shadeClose: true, //开启遮罩关闭
             content:'地址：'+'<input type="text" size="80px;" value="'+data.shareLink+'"/>'+'</br>'+
             '提取码：'+'<input type="text" size="40px;" value="'+data.shareCode+'"/>'
             });
